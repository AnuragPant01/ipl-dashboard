...
export default function UpcomingMatches({ matches }: UpcomingMatchesProps) {
  const { items, page, totalPages, loading, error, next, prev } = usePaginatedMatches("upcoming", 10);

  return (
    <Card>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
        <h2 className="font-bold text-lg sm:text-xl mb-2 sm:mb-0">Upcoming Matches</h2>
        <div className="text-sm text-gray-600">Page {page} of {totalPages}</div>
      </div>
      {error && <div className="text-sm text-red-600 mb-2 sm:mb-4">{error}</div>}
      <div className="divide-y divide-gray-200">
        {(items.length ? items : (matches || [])).map((match) => (
